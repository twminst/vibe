...
// Comprehensive Table with Sorting, Selection, and Pagination
              ...
              <Responsive
                ...
                props={{
                  ...
                  large: { 
                    ...
                    borderRadius: "medium", 
                    borderWidth: "small", borderColor: "secondary",
                    shadow: "resting"
                  }
                }}
              >
        {(cardProps) => (
          <View as="div" {...cardProps}>
            <Responsive
                  ...
                  <Table.Head renderSortLabel={<ScreenReaderContent>Sort by</ScreenReaderContent>} >
                    <Table.Row>
                      ...
                      <Table.ColHeader id="title" width="30%" textAlign="start" onRequestSort={handleSort} sortDirection={sortBy === 'title' ? direction : 'none'}>
                        {sortBy === 'title' ? ('Assignment') : (<><span aria-hidden="true">Assignment</span><ScreenReaderContent>sort by Assignment</ScreenReaderContent></>)}
                      </Table.ColHeader>
                      <Table.ColHeader id="course" width="20%" textAlign="start" onRequestSort={handleSort} sortDirection={sortBy === 'course' ? direction : 'none'}>
                        {sortBy === 'course' ? ('Course') : (<><span aria-hidden="true">Course</span><ScreenReaderContent>sort by Course</ScreenReaderContent></>)}
                      </Table.ColHeader>
                      <Table.ColHeader id="dueDate" width="15%" textAlign="start" onRequestSort={handleSort} sortDirection={sortBy === 'dueDate' ? direction : 'none'}>
                        {sortBy === 'dueDate' ? ('Due Date') : (<><span aria-hidden="true">Due Date</span><ScreenReaderContent>sort by Due Date</ScreenReaderContent></>)}
                      </Table.ColHeader>
                      <Table.ColHeader id="points" width="10%" textAlign="center" onRequestSort={handleSort} sortDirection={sortBy === 'points' ? direction : 'none'}>
                        {sortBy === 'points' ? ('Points') : (<><span aria-hidden="true">Points</span><ScreenReaderContent>sort by Points</ScreenReaderContent></>)}
                      </Table.ColHeader>
                      <Table.ColHeader id="status" width="15%" textAlign="center" onRequestSort={handleSort} sortDirection={sortBy === 'status' ? direction : 'none'}>
                        {sortBy === 'status' ? ('Status') : (<><span aria-hidden="true">Status</span><ScreenReaderContent>sort by Status</ScreenReaderContent></>)}
                      </Table.ColHeader>
                      <Table.ColHeader id="submissions" width="10%" textAlign="center" onRequestSort={handleSort} sortDirection={sortBy === 'submissions' ? direction : 'none'}>
                        {sortBy === 'submissions' ? ('Submissions') : (<><span aria-hidden="true">Submissions</span><ScreenReaderContent>sort by Submissions</ScreenReaderContent></>)}
                      </Table.ColHeader>
                    </Table.Row>
                  </Table.Head>
                  <Table.Body>
                    {paginatedData.map((row) => (
                      <Table.Row key={row.id}>
                        <Table.RowHeader>
                          <Checkbox label={<ScreenReaderContent>Select assignment {row.title}</ScreenReaderContent>} checked={selectedRows.has(row.id)} onChange={() => handleRowSelect(row.id)} />
                        </Table.RowHeader>
                        <Table.Cell textAlign={layout === 'stacked' ? 'start' : 'start'}>{row.title}</Table.Cell>
                        <Table.Cell textAlign={layout === 'stacked' ? 'start' : 'start'}>{row.course}</Table.Cell>
                        <Table.Cell textAlign={layout === 'stacked' ? 'start' : 'start'}>{new Date(row.dueDate).toLocaleDateString()}</Table.Cell>
                        <Table.Cell textAlign={layout === 'stacked' ? 'start' : 'center'}>{row.points}</Table.Cell>
                        ...
                        <Table.Cell textAlign={layout === 'stacked' ? 'start' : 'center'}>{row.submissions}</Table.Cell>
                      </Table.Row>
                    ))}
                  </Table.Body>
                </Table>
